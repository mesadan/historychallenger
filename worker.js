...
// Returns N unseen sets for a user from the pool
async function handleGetSets(body, env) {
  ...
  try {
    ...
    // Build query — filter by theme_slug if provided
    let sets;
...
// Seeds the pool — handles both random and theme seeding
async function handleSeedPool(body, env, apiKey) {
  ...
  // Get all event names already in pool for this diff/lang/theme to avoid duplicates
...
async function handleSaveSession(body, env) {
  ...
  try {
    ...
    await env.db.prepare(`INSERT INTO game_sessions (id, user_id, diff, rounds, scores, avg_score, game_type, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(sessionId, userId, diff || '', rounds || 0, JSON.stringify(scores || []), avg_score || 0, gtype, now).run();

    const user = await env.db.prepare('SELECT * FROM users WHERE id=?').bind(userId).first();
    if (user) {
      const newGames = (user.total_games || 0) + 1;
      const newAvg = ((user.avg_score || 0) * (user.total_games || 0) + (avg_score || 0)) / newGames;
      const newBest = Math.max(user.best_score || 0, avg_score || 0);
      ...
      await env.db.prepare(`UPDATE users SET total_games=?, avg_score=?, best_score=?, current_streak=?, longest_streak=?, last_streak_date=?, last_played=?, total_rounds=total_rounds+? WHERE id=?`)
        .bind(newGames, Math.round(newAvg*10)/10, newBest, streak, Math.max(user.longest_streak||0, streak), today, now, rounds||0, userId).run();
    }
    return json({ ok: true }, 200);
  } catch (e) {
...
Return ONLY the corrected set as valid JSON with exactly the same fields. No markdown, no explanation.`;
  ...
  try {
    ...
    // Validate the fix mathematically
...
async function handleSeedOverlap(body, env, apiKey) {
  ...
  // Get existing anchors to avoid repeats
...
2. "Overlap" means: civilA_start <= civB_end AND civilA_end >= civB_start
...
{"sets":[{
  ...
  try {
    ...
    for (const s of parsed.sets) {
      ...
      // All dates must be present — no nulls allowed
      ...
      // Helper: do two periods overlap?
      const overlaps = (s1, e1, s2, e2) => s1 != null && e1 != null && s2 != null && e2 != null && s1 <= e2 && e1 >= s2;

      // Correct answer MUST overlap with anchor
      if (!overlaps(s.correct_start, s.correct_end, s.anchor_start, s.anchor_end)) continue;

      // Wrong answers MUST NOT overlap with anchor — discard entire set if any do
      ...
      // Coerce all nullable integers — D1 rejects undefined
      const n = v => (v == null || v === undefined) ? null : Number(v);
