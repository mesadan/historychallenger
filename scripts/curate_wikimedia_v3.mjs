#!/usr/bin/env node
// Wikimedia v3 — same focused categories as v2, but ALSO accepts plain
// CC-BY licensed images (in addition to PD/CC0). Captures attribution
// strings into the manifest so the painting reveal card can display the
// required credit line.
//
// LICENSE POLICY:
//   ACCEPT: PD / CC0 / CC-PDM (no attribution needed)
//   ACCEPT: CC-BY 1.0 / 2.0 / 2.5 / 3.0 / 4.0 (attribution required, captured)
//   REJECT: CC-BY-SA (share-alike, would force our derivatives under SA)
//   REJECT: CC-BY-NC (non-commercial, kills monetization path)
//   REJECT: CC-BY-ND (no derivatives, we resize)
//   REJECT: All-rights-reserved, fair use
//
// De-dupes against ALL existing manifests including v1, v2, combined.
// Same prompt rejecting answer-text, generic scenes, religious devotional.
//
// Usage:
//   $env:ANTHROPIC_API_KEY="sk-ant-..."
//   node scripts/curate_wikimedia_v3.mjs
...
const CONFIG = {
  ...
  manifestFile:  'manifest_wikimedia_v3.json',
...
// Returns null (reject) or { type: 'pd' | 'cc-by', shortName: '...' }
function classifyLicense(extmetadata){
  ...
  // ── HARD REJECTS (order matters: catch SA/NC/ND before generic CC-BY match) ──
  if (/share[- ]?alike|noncommercial|non[- ]commercial|no[- ]derivatives|nonderivative|fair use|all rights reserved/.test(blob)) return null;
  if (/cc[- ]?by[- ]?(?:sa|nc|nd)/.test(blob)) return null;

  // ── PD / CC0 (no attribution required) ──
  if (/\b(cc0|cc-0|cc-?pdm|public domain|pd-?art|pd-?old|pd-?us|pd-?self|pd-?author|pd-?life|publicdomain|copyrighted free use)\b/.test(blob)) return { type: 'pd', shortName: shortNameRaw };
  if (lic === 'pd' || name === 'pd') return { type: 'pd', shortName: shortNameRaw };

  // ── CC-BY plain (attribution required) ──
  // matches "cc by", "cc-by", "cc by 4.0", "cc-by-4.0", "attribution 4.0", etc.
  // does NOT match "cc-by-sa", "cc-by-nc", etc. (already rejected above)
  if (/cc[- ]?by(?:[- ]?\d(?:\.\d)?)?(?!\w|-)/.test(blob)) return { type: 'cc-by', shortName: shortNameRaw };
  if (/^attribution(?:\s+\d(?:\.\d)?)?$/.test(name.trim())) return { type: 'cc-by', shortName: shortNameRaw };

  return null;
}

function pickAttribution(extmetadata, licType, licShortName){
  // PD images need no attribution at all
  if (licType !== 'cc-by') return null;
  // For CC-BY, build "Author / Source · LICENSE" string
  const artistRaw = extmetadata?.Artist?.value || '';
  const creditRaw = extmetadata?.Credit?.value || '';
  const cleanArtist = artistRaw.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();
  const cleanCredit = creditRaw.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();
  const author = cleanArtist || cleanCredit || 'Wikimedia Commons';
  const lic = licShortName || 'CC BY';
  return `${author.slice(0, 140)}, ${lic}`.slice(0, 200);
}
...
async function main(){
  ...
  for (const f of [
    ...
    'manifest_wikimedia.json', 'manifest_wikimedia_v2.json',
    'manifest_combined.json', CONFIG.manifestFile,
  ]){
  ...
  for (const title of candidates){
    ...
    try {
      ...
      const licenseClass = classifyLicense(info.extmetadata || {});
      if (!licenseClass){
        rejectedLicense++;
      ...
      const attribution = pickAttribution(info.extmetadata || {}, licenseClass.type, licenseClass.shortName);
      manifest.push({
        id:            baseName,
        source:        'wikimedia',
        source_id:     title,
        ...
        license_type:  licenseClass.type,    // 'pd' or 'cc-by'
        attribution:   attribution,           // null for PD, "<author>, <license>" for CC-BY
      });
